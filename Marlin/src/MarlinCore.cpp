...
const uint32 Table_Baudrate[4] = {115200, 250000, 500000, 921600};
uint8_t WiFi_BaudRate = 0;
#endif
...
#if ENABLED(OPTION_ABORT_UNLOADFILAMENT)	
	if(HMI_flag.AutoUnload_enabled && HMI_flag.AutoUnloadFilament_on) 
		queue.inject_P(PSTR(EVENT_GCODE_SD_ABORT_2));		
	else
#endif
...
void setup() {
  ...
  // (because EEPROM code calls the UI).
	...
	#if ENABLED(OPTION_WIFI_BAUDRATE)
  DISABLE_ISRS();
	WIFI_SERIAL.begin(Table_Baudrate[WiFi_BaudRate]);
