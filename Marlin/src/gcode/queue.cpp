...
/**
	* for ESP8266 WiFi Module, if got a command "M117" from WiFi, it means
...
inline int read_serial(const uint8_t index) {
  switch (index) {
    case ID_SERIAL_USB: return MYSERIAL0.read();
#if HAS_MULTI_SERIAL	
	#if HAS_MYSERIAL1
    case ID_SERIAL_MYSERIAL1: return MYSERIAL1.read();
	#endif
	#if HAS_WIFI_SERIAL
		case ID_SERIAL_WIFI: return WIFI_SERIAL.read();
	#endif 
#endif
    default: return -1;
  }
