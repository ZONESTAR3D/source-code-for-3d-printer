...
#if (defined(__AVR__) && HAS_DWIN_LCD)
constexpr uint32_t max_accl[XYZE] = DEFAULT_MAX_ACCELERATION;  		  
constexpr float steps_per_mm[XYZE] = DEFAULT_AXIS_STEPS_PER_UNIT;
constexpr feedRate_t max_feedRate[XYZE] = DEFAULT_MAX_FEEDRATE;
#else
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    #if EXTRUDERS
    {
        ...
        const fil_change_settings_t fc_settings[EXTRUDERS] = { 0, 0, 0, 0 };
      #endif
      ...
      //
      {
        ...
        if (!validating) LOOP_XYZE_N(i) {
          const bool in = (i < esteppers + XYZ);
				#if (defined(__AVR__) && HAS_DWIN_LCD)
					planner.settings.max_acceleration_mm_per_s2[i] = in ? tmp1[i] : max_accl[i];
					planner.settings.axis_steps_per_mm[i]          = in ? tmp2[i] : steps_per_mm[i];
					planner.settings.max_feedrate_mm_s[i]          = in ? tmp3[i] : max_feedRate[i];
				#else
...
void MarlinSettings::reset() {
  ...
  LOOP_XYZE_N(i) {
	#if (defined(__AVR__) && HAS_DWIN_LCD)
		planner.settings.max_acceleration_mm_per_s2[i] = max_accl[i];
		planner.settings.axis_steps_per_mm[i]          = steps_per_mm[i];
		planner.settings.max_feedrate_mm_s[i]          = max_feedRate[i];
	#else
  ...
  //
    ...
    LOOP_L_N(e, EXTRUDERS) {
      ...
      fc_settings[e].load_length = FILAMENT_CHANGE_FAST_LOAD_LENGTH;
			fc_settings[e].slow_load_length = FILAMENT_CHANGE_SLOW_LOAD_LENGTH;
			fc_settings[e].slow_unload_length = FILAMENT_UNLOAD_PURGE_FEEDRATE;
    }
  #endif
