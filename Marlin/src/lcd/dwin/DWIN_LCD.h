...
#define DWIN_SCROLL_LEFT   				0		//DONOT CHANGE IT
...
class DWINLCD{
	public:					
		...
		//void Frame_AreaCopy_Index(uint8_t PixtureID, uint8_t cacheID, uint16_t xStart, uint16_t yStart,
														// uint16_t xEnd, uint16_t yEnd, uint16_t x, uint16_t y);
		static void Frame_AreaCopy_Index(uint8_t cacheID, uint8_t PixtureID, uint8_t ItemID, const uint16_t *array, uint16_t x, uint16_t y);				
