  ...
  if (force
    ...
    info.feedrate = uint16_t(MMS_TO_MMM(feedrate_mm_s));
      ...
      info.active_extruder = active_extruder;
			...
			info.active_extruder = mixer.selected_vtool;
    #endif
...
void PrintJobRecovery::resume() {
  ...
  // Restore all hotend temperatures
    ...
    HOTEND_LOOP() {
      ...
      if (et0) {
        #if HAS_MULTI_HOTEND
          sprintf_P(cmd, PSTR("M109 S%i T%i"), et0, e);
          gcode.process_subcommands_now(cmd);
				#else
...
void PrintJobRecovery::debug(PGM_P const prefix) {
  ...
  DEBUG_ECHOLNPAIR(" Job Recovery Info...\nvalid_head:", int(info.valid_head), " valid_foot:", int(info.valid_foot));
	DEBUG_ECHOLNPAIR(" Size of Recovery file = ", sizeof(info));
  if (info.valid_head) {
        ...
        DEBUG_EOL();
			...
			DEBUG_ECHOLNPAIR("leveling: ", info.flag.leveling?1:0, " fade: ", info.fade);
				...
				MIXER_STEPPER_LOOP(i){
					...
					DEBUG_ECHOLNPAIR("] =", info.percentmix[i]);
				}
				...
				DEBUG_ECHOLNPAIR("info.gradient.end_vtool = ", info.gradient.end_vtool);
				MIXER_STEPPER_LOOP(i){
					DEBUG_ECHOPAIR("info.color[", i);
					DEBUG_ECHOLNPAIR("] =", info.gradient.color[i]);
				}
				MIXER_STEPPER_LOOP(i){
					DEBUG_ECHOPAIR("info.start_mix[", i);
					DEBUG_ECHOLNPAIR("] =", info.gradient.start_mix[i]);
				}
				MIXER_STEPPER_LOOP(i){
					DEBUG_ECHOPAIR("info.end_mix[", i);
					DEBUG_ECHOLNPAIR("] =", info.gradient.end_mix[i]);
				}
  			#endif
				...
				DEBUG_ECHOLNPAIR("info.random_mix.extruders = ", info.random_mix.extruders);
				#endif
			#endif	
