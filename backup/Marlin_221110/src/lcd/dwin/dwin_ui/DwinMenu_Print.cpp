	...
	if(encoder_diffState == ENCODER_DIFF_CW){
		...
		if(DwinMenu_print.now == PRINT_CASE_PAUSE){
			...
			if(HMI_flag.select_flag){				
				...
				if(!mixer.mixing_enabled && sum_extrude > 5 && thermalManager.temp_hotend[0].celsius > 150){ 
...
void DWIN_Draw_PrintDone_Confirm(){	
	...
	sprintf_P(timer_string, PSTR("Elapsed time:%03dH%02dM"), (int)(elapsed.hour()%100), (uint8_t)(elapsed.minute()%60));
	DWIN_Draw_MaskString_FONT12(POP_TEXT_COLOR, COLOR_BG_WINDOW, (272 - 20 * 12) / 2, 230, PSTR(timer_string));
	//
