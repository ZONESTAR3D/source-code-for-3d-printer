...
void DWIN_PopMenu_HomeDone() {
	...
	else if(DwinMenuID == DWMENU_LEVELING) Draw_Leveling_Menu(LEVELING_CASE_HOMEALL);
	dwinLCD.UpdateLCD();
}
...
#if ENABLED(AUTO_UPDATA_PROBE_Z_OFFSET)
static void Popup_Window_CatchOffset() {
	Clear_Dwin_Area(AREA_TITAL|AREA_MENU);
	Draw_Popup_Bkgd_60();
	DWIN_Show_ICON(ICON_AUTOLEVELING, 101, 105);
...
void Draw_Leveling_Menu(const uint8_t MenuItem) {
	DwinMenuID = DWMENU_LEVELING;
	DwinMenu_leveling.set(MenuItem);
	DwinMenu_leveling.index = _MAX(DwinMenu_leveling.now, MROWS);

	if(MenuItem == 0)	set_axis_never_homed(Z_AXIS);
