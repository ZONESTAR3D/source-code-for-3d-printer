...
enum {
	...
	CONFIG_CASE_COATING = CONFIG_TUNE_CASE_END,
#endif
#if ABL_GRID
...
#define	CONFIG_TUNE_CASE_TOTAL (ENABLED(FWRETRACT)+ENABLED(OPTION_REPEAT_PRINTING)+ENABLED(FILAMENT_RUNOUT_SENSOR)+ENABLED(POWER_LOSS_RECOVERY)+ ENABLED(OPTION_AUTOPOWEROFF)+ENABLED(OPTION_WIFI_MODULE)+ENABLED(OPTION_WIFI_BAUDRATE))
#define	CONFIG_CASE_TOTAL (CONFIG_TUNE_CASE_TOTAL+ENABLED(OPTION_BED_COATING)+ENABLED(OPTION_HOTENDMAXTEMP)+ EITHER(PID_EDIT_MENU, PID_AUTOTUNE_MENU) + ABL_GRID*2+ENABLED(RGB_LED))
