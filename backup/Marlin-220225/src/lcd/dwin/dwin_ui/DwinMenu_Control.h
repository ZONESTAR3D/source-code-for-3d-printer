...
enum {
	...
	CONFIG_CASE_WIFISPEED = CONFIG_TUNE_CASE_END,
#endif
...
#define	CONFIG_TUNE_CASE_TOTAL (ENABLED(FWRETRACT)+ENABLED(OPTION_REPEAT_PRINTING)+ENABLED(FILAMENT_RUNOUT_SENSOR)+ENABLED(POWER_LOSS_RECOVERY)+ ENABLED(OPTION_AUTOPOWEROFF)+ENABLED(OPTION_WIFI_MODULE))
#define	CONFIG_CASE_TOTAL (CONFIG_TUNE_CASE_TOTAL + BOTH(OPTION_WIFI_MODULE, OPTION_WIFI_BAUDRATE) + BOTH(MIXING_EXTRUDER, OPTION_MIXING_SWITCH) + ENABLED(OPTION_BED_COATING) + ENABLED(OPTION_HOTENDMAXTEMP) + EITHER(PID_EDIT_MENU, PID_AUTOTUNE_MENU) + ABL_GRID*2)
