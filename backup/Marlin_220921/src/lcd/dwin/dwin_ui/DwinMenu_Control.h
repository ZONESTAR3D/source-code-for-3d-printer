...
enum {
	...
	CONFIG_CASE_HOMEZOFFSET,
#endif
#if ENABLED(HAS_OFFSET_MENU)
...
#define	CONFIG_CASE_TOTAL (CONFIG_TUNE_CASE_TOTAL + BOTH(OPTION_WIFI_MODULE, OPTION_WIFI_BAUDRATE) + ENABLED(SWITCH_EXTRUDER_MENU) + BOTH(MIXING_EXTRUDER, OPTION_MIXING_SWITCH) + ENABLED(OPTION_HOMEZ_OFFSET) + HAS_OFFSET_MENU + ENABLED(OPTION_HOTENDMAXTEMP) + EITHER(PID_EDIT_MENU, PID_AUTOTUNE_MENU) + ABL_GRID*2)
