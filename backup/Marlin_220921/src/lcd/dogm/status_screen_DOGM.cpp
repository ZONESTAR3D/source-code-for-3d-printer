...
FORCE_INLINE void _draw_mixetruder_status() {	
	...
	strcpy(str, i8tostr3rj(mixer.percentmix[1]));
	len = str[0] != ' ' ? 3 : str[1] != ' ' ? 2 : 1;
	if(len == 1)
		x = 35;
	else if(len == 2)
	...
	strcpy(str, i8tostr3rj(mixer.percentmix[2]));
	len = str[0] != ' ' ? 3 : str[1] != ' ' ? 2 : 1;
	if(len == 1)
		x = 38;
	else if(len == 2)
	...
	strcpy(str, i8tostr3rj(mixer.percentmix[3]));
	len = str[0] != ' ' ? 3 : str[1] != ' ' ? 2 : 1;
	if(len == 1)
		x = 55;
	else if(len == 2)
...
void MarlinUI::draw_status_screen() {
  ...
  if (PAGE_CONTAINS(XYZ_FRAME_TOP, XYZ_FRAME_TOP + XYZ_FRAME_HEIGHT - 1)) {
    ...
    if (PAGE_CONTAINS(XYZ_BASELINE - (INFO_FONT_ASCENT - 1), XYZ_BASELINE)) {
        ...
        sprintf_P(mixer_messages, PSTR(S_FMT " %d:%d%% "), mix_label, int(mixer.percentmix[0]), int(mixer.percentmix[1]));
